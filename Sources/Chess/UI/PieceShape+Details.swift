            ...
            return Path { path in
                for highlight in artwork.highlights {
                    ...
                    guard let control1 = stroke.control1 else {
                        // It's just a line
                        path.addLine(to: stroke.end.mapped(in: rect, inset: inset))
                        continue
                    }
                    guard let control2 = stroke.control2 else {
                        // It's a single control curve
                        path.addQuadCurve(to: stroke.end.mapped(in: rect, inset: inset),
                                          control: control1.mapped(in: rect, inset: inset))
                        continue
                    }
                    path.addCurve(to: stroke.end.mapped(in: rect, inset: inset),
